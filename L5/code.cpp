...
Возврат: 1 - было введено 'y', 0 - было введено 'n'.
*/
bool input_yn()
{
...
Возврат: 1 - символ разрешон иначе, 0.
*/
...
/*
Описание: Это функция для получения полей структуры.
Возврат: Эта функция ничего не возвращает.
*/
...
void main()
{
	...
	do
	{
		...
		puts("1-добавление элемента в очередь");
		puts("2-взятие элемента из очереди");
		puts("3-вывод элементов очереди");
		puts("4-удаление очереди из оперативной памяти");
		puts("5-справка");
		puts("0-выход");
		menu = input_number();
		system("cls");
